...
<template>
  <div>
    ...
    <!-- <food :food="food" v-if="isShow"></food> -->
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(['goods']),
    // 计算得到当前分类的下标
    currentIndex () { // 初始和相关数据发生了变化才执行此函数
      // 得到条件数据
      const {scrollY, tops} = this
      // 根据条件计算产生一个结果 indexOf()找数组中某一个元素的下标
      const index = tops.findIndex((top, index) => { // 首先此方法在做遍历
  ...
  methods: { // 一般放事件回调函数相关的方法
    ...
    _initScrool () {
      // 列表显示后创建
        ...
        this.foodsScroll.on('scroll', ({x, y}) => {
          console.log(`Now position is x: ${x}, y: ${y}`)
    ...
    _initTops () {
      ...
      const lis = this.$refs.foodsUl.getElementsByClassName('food-list-hook') // 得到一个伪数组
